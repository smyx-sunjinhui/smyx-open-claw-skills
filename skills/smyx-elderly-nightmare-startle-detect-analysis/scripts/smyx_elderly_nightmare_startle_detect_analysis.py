...
def main():
    parser = argparse.ArgumentParser(description="老年人睡眠中间惊醒/梦魇行为识别工具")
    parser.add_argument("--input", help="本地卧室固定摄像头（红外夜视、含麦克风）夜间睡眠音视频文件路径")
    parser.add_argument("--url", help="网络卧室固定摄像头（红外夜视、含麦克风）夜间睡眠音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人睡眠行为监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示老年人睡眠中间惊醒/梦魇行为识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在检测夜间突然坐起/惊叫声/无目的挥臂等异常睡眠事件并统计频次与时长，请稍候...")
        ...
        print(f"❌ 老年人睡眠中间惊醒/梦魇行为识别失败: {str(e)}")
        exit(1)
