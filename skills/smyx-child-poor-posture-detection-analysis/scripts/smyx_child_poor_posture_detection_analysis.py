...
def main():
    parser = argparse.ArgumentParser(description="儿童坐姿不良（驼背/歪头）实时提醒工具")
    parser.add_argument("--input", help="本地儿童学习区域坐姿监控视频文件路径")
    parser.add_argument("--url", help="网络儿童学习区域坐姿监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识:儿童健康坐姿场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示儿童坐姿不良识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析儿童坐姿（驼背/歪头）并生成实时提醒，请稍候...")
