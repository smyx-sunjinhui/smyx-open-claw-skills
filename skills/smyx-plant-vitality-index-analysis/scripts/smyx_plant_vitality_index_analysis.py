...
def main():
    parser = argparse.ArgumentParser(description="植物整体活力指数（综合评分）工具")
    parser.add_argument("--input", help="本地植物图像/连续日间图像序列/视频文件路径")
    parser.add_argument("--url", help="网络植物图像/连续日间图像序列/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：植物活力评估场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示植物活力指数分析列表清单")
