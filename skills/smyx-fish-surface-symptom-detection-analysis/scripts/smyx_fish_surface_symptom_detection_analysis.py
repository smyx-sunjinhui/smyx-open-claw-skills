...
def main():
    parser = argparse.ArgumentParser(description="鱼类体表白点/充血/烂尾识别工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头/水下摄像头高清图像或视频文件路径")
    parser.add_argument("--url", help="网络鱼缸固定摄像头/水下摄像头高清图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类体表症状识别场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（养鱼用户或场馆授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类体表症状识别历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测鱼体表症状（白点/充血/烂尾）并输出置信度，请稍候...")
