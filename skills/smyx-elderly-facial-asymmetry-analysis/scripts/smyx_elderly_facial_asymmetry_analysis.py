...
def main():
    parser = argparse.ArgumentParser(description="老年人面部不对称/口角歪斜识别工具")
    parser.add_argument("--input", help="本地老年人正面面部图像/视频文件路径")
    parser.add_argument("--url", help="网络老年人正面面部图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人健康监测场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人面部不对称识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析老年人面部对称性并计算不对称指数，请稍候...")
        ...
        print(f"❌ 老年人面部不对称/口角歪斜识别失败: {str(e)}")
        exit(1)
