...
def main():
    parser = argparse.ArgumentParser(description="青少年沉迷手机/游戏行为识别工具")
    parser.add_argument("--input", help="本地家庭/自习室/学校固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络家庭/自习室/学校固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：青少年健康行为监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示青少年沉迷手机/游戏行为识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在检测头部俯仰角与手持设备姿势，统计连续低头与日累计看屏幕时长，请稍候...")
        ...
        print(f"❌ 青少年沉迷手机/游戏行为识别失败: {str(e)}")
        exit(1)
