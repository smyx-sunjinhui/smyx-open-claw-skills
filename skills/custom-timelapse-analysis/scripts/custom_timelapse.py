...
def main():
    parser = argparse.ArgumentParser(description="时光相册自定义浓缩分析工具")
    parser.add_argument("--input", help="本地视频文件路径")
    parser.add_argument("--url", help="网络视频URL地址")
    parser.add_argument("--text", help="自定义提取目标/关键词描述")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示时光相册浓缩分析列表清单")
    ...
    try:
        ...
        if not args.text:
            print("⚠️  提示：未提供自定义提取目标/关键词描述，建议使用 --text 参数说明需要提取什么内容")

        print("🔍 正在浓缩时光相册，请稍候...")
        output_content = analyze_timelapse(
