...
def main():
    parser = argparse.ArgumentParser(description="鱼卵孵化状态识别工具")
    parser.add_argument("--input", help="本地繁殖缸固定摄像头/微距镜头鱼卵高清图像或视频文件路径")
    parser.add_argument("--url", help="网络繁殖缸固定摄像头/微距镜头鱼卵高清图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼卵孵化状态识别场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（繁殖者/育苗场/实验室授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼卵孵化状态识别历史记录清单")
    ...
    try:
        ...
        print("🔍 正在通过微距图像检测鱼卵颜色变化与胚胎眼睛点，判定孵化阶段，请稍候...")
