...
def main():
    parser = argparse.ArgumentParser(description="抑郁症辅助行为标记（长时间不动、食欲改变）工具")
    parser.add_argument("--input", help="本地家庭固定摄像头（卧室+餐厅区域，≥24h）视频文件路径")
    parser.add_argument("--url", help="网络家庭固定摄像头（卧室+餐厅区域，≥24h）视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人行为健康监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示抑郁症辅助行为标记分析列表清单")
    ...
    try:
        ...
        print("🔍 正在统计每日卧床时长与进食动作次数/时长，并与个人基线对比，请稍候...")
