...
def main():
    parser = argparse.ArgumentParser(description="职场员工压力群体热力图工具")
    parser.add_argument("--input", help="本地办公区固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络办公区固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：职场健康分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示职场员工压力群体热力图分析列表清单")
    ...
    try:
        ...
        print("🔍 正在匿名分析办公区员工面部紧张特征与姿态僵硬度，并生成区域级压力热力图，请稍候...")
