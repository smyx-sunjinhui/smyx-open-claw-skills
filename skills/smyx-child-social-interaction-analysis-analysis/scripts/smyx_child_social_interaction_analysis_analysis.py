...
def main():
    parser = argparse.ArgumentParser(description="儿童社交互动频次与时长分析工具")
    parser.add_argument("--input", help="本地幼儿园/游乐场多人场景视频文件路径")
    parser.add_argument("--url", help="网络幼儿园/游乐场多人场景视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童社交行为分析场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示儿童社交互动分析列表清单")
