...
def main():
    parser = argparse.ArgumentParser(description="鱼类擦缸/蹭底行为识别（外寄）工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头视频文件路径（建议帧率 ≥ 25 FPS，覆盖缸壁+底砂全景）")
    parser.add_argument("--url", help="网络鱼缸固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类擦缸/蹭底外寄场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养鱼者/水族馆/检疫缸/养殖池管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类擦缸/蹭底外寄风险预警历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测鱼体侧身蹭缸壁/腹部蹭底砂等异常摩擦行为，统计每分钟频次评估外寄风险，请稍候...")
