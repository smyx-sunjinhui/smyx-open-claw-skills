...
def main():
    parser = argparse.ArgumentParser(description="成人久坐/姿态预警（办公室）工具")
    parser.add_argument("--input", help="本地办公工位监控视频文件路径")
    parser.add_argument("--url", help="网络办公工位监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：职场健康管理场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示成人久坐/姿态预警分析列表清单")
