...
def main():
    parser = argparse.ArgumentParser(description="鱼类游动姿态异常（侧游/倒立）识别工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头视频文件路径")
    ...
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类游姿监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（养鱼用户或场馆授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类游姿异常监测历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测鱼类游姿异常（侧游/倒立/翻转）并统计异常时长占比，请稍候...")
