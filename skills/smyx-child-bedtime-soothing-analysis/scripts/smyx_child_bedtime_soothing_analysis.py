...
def main():
    parser = argparse.ArgumentParser(description="儿童睡前情绪安抚（怕黑/噩梦后）工具")
    parser.add_argument("--input", help="本地儿童卧室/婴儿房固定摄像头（红外夜视+麦克风）夜间音视频文件路径")
    parser.add_argument("--url", help="网络儿童卧室/婴儿房固定摄像头（红外夜视+麦克风）夜间音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童睡眠安抚场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（家长授权）")
    parser.add_argument("--list", action='store_true', help="显示儿童睡前情绪安抚（怕黑/噩梦后）历史安抚记录清单")
    ...
    try:
        ...
        print("🔍 正在检测睡前哭闹/怕黑蜷缩/噩梦惊醒等不安信号并触发安抚动作，请稍候...")
        ...
        print(f"❌ 儿童睡前情绪安抚（怕黑/噩梦后）失败: {str(e)}")
        exit(1)
