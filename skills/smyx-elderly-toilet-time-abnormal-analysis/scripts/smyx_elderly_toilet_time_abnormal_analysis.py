...
def main():
    parser = argparse.ArgumentParser(description="老年人如厕时间异常（超30分钟）识别工具")
    parser.add_argument("--input", help="本地卫生间门口/内部隐私化人体监控视频文件路径")
    parser.add_argument("--url", help="网络卫生间门口/内部隐私化人体监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老人居家安全场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示老人如厕时间异常识别分析列表清单")
