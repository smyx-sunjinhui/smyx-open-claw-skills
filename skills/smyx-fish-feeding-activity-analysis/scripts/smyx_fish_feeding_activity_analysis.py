...
def main():
    parser = argparse.ArgumentParser(description="鱼类摄食行为活跃度分析工具")
    parser.add_argument("--input", help="本地智能喂食器/鱼缸固定摄像头投喂后1分钟内视频文件路径")
    parser.add_argument("--url", help="网络智能喂食器/鱼缸固定摄像头投喂后1分钟内视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类摄食活跃度分析场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养鱼用户、养殖场或场馆授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类摄食行为活跃度分析历史记录清单")
    ...
    try:
        ...
        print("🔍 正在分析鱼群摄食活跃度（聚集数量/游动速度/摄食动作频率/剩余饲料），生成 0-100 评分，请稍候...")
