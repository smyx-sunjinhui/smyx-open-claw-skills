...
def main():
    parser = argparse.ArgumentParser(description="独居者作息规律异常分析工具")
    parser.add_argument("--input", help="本地独居者卧室/客厅夜间视频文件路径（建议覆盖22:00-6:00）")
    parser.add_argument("--url", help="网络独居者卧室/客厅夜间视频的URL地址（建议覆盖22:00-6:00）")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：独居者作息监测场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示独居者作息规律异常分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析独居者熄灯时间与凌晨活动并对比历史基线，请稍候...")
