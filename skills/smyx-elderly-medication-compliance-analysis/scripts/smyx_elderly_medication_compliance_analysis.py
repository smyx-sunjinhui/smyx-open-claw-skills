...
def main():
    parser = argparse.ArgumentParser(description="老年人服药动作确认（取药/入口/吞咽）工具")
    parser.add_argument("--input", help="本地药箱区域服药全过程监控视频文件路径")
    parser.add_argument("--url", help="网络药箱区域服药全过程监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老人用药安全场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人服药动作依从性分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析服药动作（取药/入口/吞咽）并判断依从性，请稍候...")
