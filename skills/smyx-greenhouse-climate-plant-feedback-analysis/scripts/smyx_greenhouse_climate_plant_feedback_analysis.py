...
def main():
    parser = argparse.ArgumentParser(description="温室环境与植物状态联动调控工具")
    parser.add_argument("--input", help="本地温室植物图像/视频文件路径")
    parser.add_argument("--url", help="网络温室植物图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：温室植物场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示温室联动调控分析列表清单")
