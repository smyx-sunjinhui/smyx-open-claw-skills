...
def main():
    parser = argparse.ArgumentParser(description="爬宠排泄物形态识别（尿酸/粪便）工具")
    parser.add_argument("--input",
                        help="本地爬宠箱排泄物高清图像或视频静态帧文件路径（建议俯拍，含已知尺寸参考物，分辨率≥1080p）")
    parser.add_argument("--url", help="网络爬宠箱排泄物高清图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：爬宠排泄物场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/养殖场/爬宠医院授权）")
    parser.add_argument("--list", action='store_true', help="显示爬宠排泄物历史评估记录清单")
    ...
    try:
        ...
        print("🔍 正在分析爬宠排泄物：尿酸面积/颜色 + 粪便颜色/形态/血便，对比物种正常范围，评估肾脏与肠道健康，请稍候...")
