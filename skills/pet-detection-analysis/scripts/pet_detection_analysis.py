...
def format_result(result, output_level="standard", media_type="video", confidence_threshold=0.5):
    ...
    if output_level == "json":
        ...
        if result is not None:
            ...
            result_json = json.dumps(result_json.get('petDetectionResponse', {}), ensure_ascii=False, indent=2)
        else:
            return "⚠️ 暂无分析结果"
        return f"""
📊 宠物检测分析结构化结果
...
猫: {detection.get('cat_count', 0)} 只
狗: {detection.get('dog_count', 0)} 只
鸟: {detection.get('bird_count', 0)} 只
        """
...
🔍 检测结果:
  ...
  各类宠物统计:
{objects if objects else '  ✅ 未检测到宠物'}
{'=' * 50}
> 注：本报告仅供家庭宠物监控参考。
        """
    else:
...
def main():
    parser = argparse.ArgumentParser(description="宠物检测工具")
    ...
    parser.add_argument("--list", action='store_true', help="显示宠物检测列表清单")
