...
def main():
    parser = argparse.ArgumentParser(description="老年人厨房忘关火识别工具")
    parser.add_argument("--input", help="本地厨房灶台区域监控视频文件路径")
    parser.add_argument("--url", help="网络厨房灶台区域监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：厨房安全场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示厨房忘关火识别分析列表清单")
