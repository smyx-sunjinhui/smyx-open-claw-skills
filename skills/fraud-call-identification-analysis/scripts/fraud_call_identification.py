...
def main():
    parser = argparse.ArgumentParser(description="诈骗电话识别分析工具")
    parser.add_argument("--input", help="本地音频文件路径")
    parser.add_argument("--url", help="网络音频URL地址")
    parser.add_argument("--text", help="通话文字内容")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示诈骗电话识别分析列表清单")
    ...
    try:
        ...
        if not args.input and not args.url and not args.text:
            print("❌ 错误: 必须提供 --input、--url 或 --text 参数")
            exit(1)

        print("🔍 正在分析通话风险，请稍候...")
        output_content = analyze_call(
