...
def main():
    parser = argparse.ArgumentParser(description="老年人饮水杯拿起频率（脱水风险）工具")
    parser.add_argument("--input", help="本地客厅/厨房水杯区域视频文件路径")
    parser.add_argument("--url", help="网络客厅/厨房水杯区域视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人健康护理场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人饮水频率（脱水风险）分析列表清单")
    ...
    try:
        ...
        print("🔍 正在统计老年人手部与水杯接触动作并评估每日饮水频率/脱水风险，请稍候...")
