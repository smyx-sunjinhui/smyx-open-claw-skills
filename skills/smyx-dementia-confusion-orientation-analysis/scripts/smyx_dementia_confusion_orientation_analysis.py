...
def main():
    parser = argparse.ArgumentParser(description="失智老人困惑/迷惘识别与定向安抚工具")
    parser.add_argument("--input", help="本地失智照护机构/家庭固定摄像头（可选麦克风）音视频文件路径")
    parser.add_argument("--url", help="网络失智照护机构/家庭固定摄像头（可选麦克风）音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：失智老人定向安抚场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（家属或机构授权）")
    parser.add_argument("--list", action='store_true', help="显示失智老人困惑/迷惘识别与定向安抚历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测失智老人困惑/迷惘信号（突然停止活动/眼神游离/四处张望/重复定向问题）并触发定向安抚，请稍候...")
        ...
        print(f"❌ 失智老人困惑/迷惘识别与定向安抚失败: {str(e)}")
        exit(1)
