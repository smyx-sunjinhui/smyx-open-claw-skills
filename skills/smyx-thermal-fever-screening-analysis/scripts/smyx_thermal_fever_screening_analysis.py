...
def main():
    parser = argparse.ArgumentParser(description="家庭多人聚集时体温相对异常检测工具")
    parser.add_argument("--input", help="本地热成像摄像头多人聚集视频文件路径")
    parser.add_argument("--url", help="网络热成像摄像头多人聚集视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：群体体温筛查场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示家庭多人聚集体温相对异常分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析多人聚集场景下的额头体表温度并计算个体与群体均值差，请稍候...")
        ...
        print(f"❌ 家庭多人聚集体温相对异常检测失败: {str(e)}")
        exit(1)
