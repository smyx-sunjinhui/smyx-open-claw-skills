...
def main():
    parser = argparse.ArgumentParser(description="儿童攀爬窗户/阳台识别工具")
    parser.add_argument("--input", help="本地窗户/阳台区域监控视频文件路径")
    parser.add_argument("--url", help="网络窗户/阳台区域监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童居家安全场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示儿童攀爬窗户/阳台识别分析列表清单")
