...
def main():
    parser = argparse.ArgumentParser(description="儿童专注度与走神时段分析工具")
    parser.add_argument("--input", help="本地儿童学习区域监控视频文件路径")
    parser.add_argument("--url", help="网络儿童学习区域监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童学习行为分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示儿童专注度分析列表清单")
