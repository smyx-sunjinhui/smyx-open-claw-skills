...
def main():
    parser = argparse.ArgumentParser(description="番茄/草莓果实成熟度分级工具")
    parser.add_argument("--input", help="本地果实图像/视频文件路径")
    parser.add_argument("--url", help="网络果实图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：果蔬场景默认 other")
