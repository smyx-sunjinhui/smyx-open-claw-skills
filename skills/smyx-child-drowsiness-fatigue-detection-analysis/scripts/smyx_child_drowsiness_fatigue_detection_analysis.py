...
def main():
    parser = argparse.ArgumentParser(description="儿童打瞌睡/疲劳检测工具")
    parser.add_argument("--input", help="本地儿童面部学习区域视频文件路径")
    parser.add_argument("--url", help="网络儿童面部学习区域视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童学习疲劳监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示儿童打瞌睡/疲劳检测分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析儿童疲劳指数（眼部闭合/点头）并生成提醒，请稍候...")
