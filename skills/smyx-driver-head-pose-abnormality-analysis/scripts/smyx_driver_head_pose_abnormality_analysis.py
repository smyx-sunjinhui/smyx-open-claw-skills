...
def main():
    parser = argparse.ArgumentParser(description="驾驶员头部姿态异常（低头/侧视）检测工具")
    parser.add_argument("--input", help="本地车载DMS驾驶员面部视频文件路径")
    parser.add_argument("--url", help="网络车载DMS驾驶员面部视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：驾驶员分心监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示驾驶员头部姿态异常分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析驾驶员头部姿态（俯仰/偏航/翻滚角 + 低头/侧视持续时间），请稍候...")
