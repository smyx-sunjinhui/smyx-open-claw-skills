...
def main():
    parser = argparse.ArgumentParser(description="新生儿黄疸筛查（面部皮肤颜色）工具")
    parser.add_argument("--input", help="本地新生儿面部高清图像/短视频文件路径")
    parser.add_argument("--url", help="网络新生儿面部高清图像/短视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：新生儿健康筛查场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示新生儿黄疸筛查分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析新生儿巩膜颜色与面部皮肤黄染指数并估算黄疸风险等级，请稍候...")
