...
def main():
    parser = argparse.ArgumentParser(description="婴幼儿趴睡窒息风险识别工具")
    parser.add_argument("--input", help="本地婴儿床上方监控视频文件路径")
    parser.add_argument("--url", help="网络婴儿床上方监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：婴儿睡眠安全场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示婴儿趴睡窒息风险识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在识别婴儿睡姿与口鼻遮挡，评估窒息风险，请稍候...")
