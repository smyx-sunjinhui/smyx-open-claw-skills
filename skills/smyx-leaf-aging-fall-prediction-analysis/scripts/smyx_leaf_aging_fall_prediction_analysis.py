...
def main():
    parser = argparse.ArgumentParser(description="室内绿植叶片老化/脱落预测工具")
    parser.add_argument("--input", help="本地室内绿植叶片图像序列/视频文件路径")
    parser.add_argument("--url", help="网络室内绿植叶片图像序列/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：植物场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示叶片老化/脱落预测分析列表清单")
