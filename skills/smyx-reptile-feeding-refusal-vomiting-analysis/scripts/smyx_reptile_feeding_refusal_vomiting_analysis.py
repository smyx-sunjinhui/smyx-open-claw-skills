...
def main():
    parser = argparse.ArgumentParser(description="爬宠进食拒绝/呕吐识别工具")
    parser.add_argument("--input", help="本地爬宠箱喂食视频文件路径（建议覆盖投喂瞬间→吞食→喂食后2小时全程）")
    parser.add_argument("--url", help="网络爬宠箱喂食视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：爬宠拒食/呕吐场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/爬宠养殖场管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示爬宠拒食/呕吐异常事件历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测爬宠的攻击/吞食/反吐行为，识别拒食与呕吐异常事件，请稍候...")
