...
def main():
    parser = argparse.ArgumentParser(description="鱼苗生长速度测量（通过参照物）工具")
    parser.add_argument("--input", help="本地鱼苗缸固定摄像头含参照物（刻度尺/标准硬币/标定板）的高清图像或视频文件路径")
    parser.add_argument("--url", help="网络鱼苗缸固定摄像头含参照物的高清图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼苗生长速度测量场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养殖者/繁育者/实验室授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼苗生长速度测量历史记录清单（含历次体长 + 生长曲线）")
    ...
    try:
        ...
        print("🔍 正在通过参照物（刻度尺/标准硬币/标定板）将鱼苗像素长度换算为实际 mm，并计算生长速率，请稍候...")
