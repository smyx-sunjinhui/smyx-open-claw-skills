...
def main():
    parser = argparse.ArgumentParser(description="学生课堂情绪参与度分析工具")
    parser.add_argument("--input", help="本地教室固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络教室固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：课堂教学分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示学生课堂情绪参与度分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析学生课堂面部表情，计算班级整体参与度评分并匿名提示低参与度位置，请稍候...")
