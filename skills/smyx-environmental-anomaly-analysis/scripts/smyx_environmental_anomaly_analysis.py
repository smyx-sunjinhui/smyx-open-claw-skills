...
def main():
    parser = argparse.ArgumentParser(description="畜禽舍环境异常联动工具")
    parser.add_argument("--input", help="本地畜禽舍图像/视频/传感器数据文件路径")
    parser.add_argument("--url", help="网络畜禽舍图像/视频/传感器数据URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：cat, dog, other，默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示畜禽舍环境异常联动历史报告清单")
