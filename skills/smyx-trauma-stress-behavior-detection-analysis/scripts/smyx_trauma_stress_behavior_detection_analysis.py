...
def main():
    parser = argparse.ArgumentParser(description="受灾人群心理创伤行为识别（应急场景）工具")
    parser.add_argument("--input", help="本地应急避难所/临时安置点固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络应急避难所/临时安置点固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：灾后心理危机识别场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（应急指挥中心/心理救援团队授权）")
    parser.add_argument("--list", action='store_true', help="显示受灾人群心理创伤行为识别历史预警清单")
    ...
    try:
        ...
        print("🔍 正在检测木僵/颤抖/无反应/过度警觉等急性应激行为信号并定位区域，请稍候...")
