...
def main():
    parser = argparse.ArgumentParser(description="老年人手部震颤（静止性）识别工具")
    parser.add_argument("--input", help="本地老年人手部静止状态视频文件路径")
    parser.add_argument("--url", help="网络老年人手部静止状态视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人神经系统健康监测场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人手部震颤识别分析列表清单")
