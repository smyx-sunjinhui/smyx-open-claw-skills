...
def main():
    parser = argparse.ArgumentParser(description="员工情绪波动 HR 报告工具")
    parser.add_argument("--input", help="本地企业办公区固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络企业办公区固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：职场心理健康监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（仅限HR管理层）")
    parser.add_argument("--list", action='store_true', help="显示员工情绪波动 HR 报告列表清单")
    ...
    try:
        ...
        print("🔍 正在以匿名ID跟踪员工，统计笑容/叹气/皱眉/独自静坐与基线对比并生成情绪波动预警，请稍候...")
        ...
        print(f"❌ 员工情绪波动 HR 报告生成失败: {str(e)}")
        exit(1)
