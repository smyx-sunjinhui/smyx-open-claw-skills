...
def main():
    parser = argparse.ArgumentParser(description="水培植物营养液浓度视觉评估工具")
    parser.add_argument("--input", help="本地水培植物根系/叶片图像或视频文件路径")
    parser.add_argument("--url", help="网络水培植物根系/叶片图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：水培植物场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示水培营养液浓度评估列表清单")
