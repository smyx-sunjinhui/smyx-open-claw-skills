...
def main():
    parser = argparse.ArgumentParser(description="儿童开心时刻识别与正向激励工具")
    parser.add_argument("--input", help="本地家庭客厅/幼儿园教室/游乐场/亲子活动中心固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络家庭客厅/幼儿园教室/游乐场/亲子活动中心固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童积极情绪记录场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（家长/教师授权）")
    parser.add_argument("--list", action='store_true', help="显示儿童开心时刻识别与正向激励历史快乐合集清单")
    ...
    try:
        ...
        print("🔍 正在识别大笑/蹦跳/拍手/笑声等开心瞬间并自动抓拍片段，请稍候...")
