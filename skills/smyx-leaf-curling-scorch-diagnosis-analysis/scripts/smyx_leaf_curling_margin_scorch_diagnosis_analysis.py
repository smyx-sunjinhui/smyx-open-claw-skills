...
def main():
    parser = argparse.ArgumentParser(description="植物卷叶/焦边识别（干旱/病害）工具")
    parser.add_argument("--input", help="本地植物叶片图像或视频文件路径")
    parser.add_argument("--url", help="网络植物叶片图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：植物逆境诊断场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示卷叶/焦边诊断分析列表清单")
