...
def main():
    parser = argparse.ArgumentParser(description="自闭症儿童刻板行为识别（转圈/摆手）工具")
    parser.add_argument("--input", help="本地康复/家庭儿童行为视频文件路径")
    parser.add_argument("--url", help="网络康复/家庭儿童行为视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：自闭症儿童行为分析场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示自闭症儿童刻板行为识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在识别儿童重复性刻板动作（转圈/摆手/摇晃）并统计频次/持续时间，请稍候...")
