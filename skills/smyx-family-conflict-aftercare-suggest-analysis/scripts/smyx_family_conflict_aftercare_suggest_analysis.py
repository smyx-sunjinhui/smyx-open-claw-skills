...
def main():
    parser = argparse.ArgumentParser(description="夫妻/家人冲突后情绪缓和提示工具")
    parser.add_argument("--input", help="本地家庭客厅/厨房/餐厅固定摄像头（含麦克风）音视频文件路径")
    parser.add_argument("--url", help="网络家庭客厅/厨房/餐厅固定摄像头（含麦克风）音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：家庭情绪缓和场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（家庭主用户授权）")
    parser.add_argument("--list", action='store_true', help="显示夫妻/家人冲突后情绪缓和提示历史事件清单")
    ...
    try:
        ...
        print("🔍 正在检测高分贝争吵/摔门/甩手等冲突事件并判定平静窗口，请稍候...")
        ...
        print(f"❌ 夫妻/家人冲突后情绪缓和提示失败: {str(e)}")
        exit(1)
