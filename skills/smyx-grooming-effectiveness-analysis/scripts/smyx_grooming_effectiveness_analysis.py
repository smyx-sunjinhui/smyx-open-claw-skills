...
def main():
    parser = argparse.ArgumentParser(description="宠物梳毛器梳理效果与毛球风险分析工具")
    parser.add_argument("--input", help="本地梳毛器视频/图片文件路径")
    parser.add_argument("--url", help="网络梳毛器视频/图片的URL地址")
    ...
    parser.add_argument("--list", action='store_true', help="显示梳毛器梳理效果历史分析列表清单")
    ...
    try:
        ...
        print(f"❌ 梳毛器梳理效果与毛球风险分析失败: {str(e)}")
        exit(1)
