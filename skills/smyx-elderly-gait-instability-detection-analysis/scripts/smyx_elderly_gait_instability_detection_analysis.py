...
def main():
    parser = argparse.ArgumentParser(description="老年人步态不稳/小碎步识别工具")
    parser.add_argument("--input", help="本地老年人直线行走视频文件路径")
    parser.add_argument("--url", help="网络老年人直线行走视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人步态安全场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示老年人步态不稳识别分析列表清单")
