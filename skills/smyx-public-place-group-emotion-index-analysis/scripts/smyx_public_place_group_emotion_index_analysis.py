...
def main():
    parser = argparse.ArgumentParser(description="公共场所群体情绪指数（展览/商场）工具")
    parser.add_argument("--input", help="本地公共场所固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络公共场所固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：公共场所群体情绪分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示公共场所群体情绪指数分析列表清单")
    ...
    try:
        ...
        print("🔍 正在匿名统计画面中多人的情绪分布并计算群体情绪指数，请稍候...")
