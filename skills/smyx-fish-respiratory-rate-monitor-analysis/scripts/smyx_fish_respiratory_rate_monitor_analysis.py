...
def main():
    parser = argparse.ArgumentParser(description="鱼类呼吸频率（鳃盖开合）监测工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头鱼鳃盖近距离视频文件路径")
    parser.add_argument("--url", help="网络鱼缸固定摄像头鱼鳃盖近距离视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类呼吸频率监测场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养鱼用户、场馆或实验室授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类呼吸频率监测历史记录清单")
    ...
    try:
        ...
        print("🔍 正在通过鳃盖开合周期计算鱼类呼吸频率（次/分钟）并判定缺氧预警，请稍候...")
