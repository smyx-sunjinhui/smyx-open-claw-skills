...
def main():
    parser = argparse.ArgumentParser(description="爬宠体温调节行为识别（晒点/躲避）工具")
    parser.add_argument("--input", help="本地爬宠箱固定摄像头视频文件路径（建议俯拍或斜俯拍覆盖晒点+躲避区+冷区全景）")
    parser.add_argument("--url", help="网络爬宠箱固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：爬宠体温调节行为场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/爬宠养殖场管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示爬宠温区利用历史报告清单")
    ...
    try:
        ...
        print("🔍 正在检测爬宠在晒点/躲避区/冷区之间的移动频次与停留时长，评估温区偏好与活动节律，请稍候...")
