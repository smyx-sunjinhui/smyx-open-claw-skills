...
def main():
    parser = argparse.ArgumentParser(description="经济作物专属病害识别工具")
    parser.add_argument("--input", help="本地作物叶片图像/视频文件路径")
    parser.add_argument("--url", help="网络作物叶片图像/视频URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识（作物场景保持 other）")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示经济作物病害识别历史报告清单")
