...
def main():
    parser = argparse.ArgumentParser(description="爬宠蜕皮进度识别工具")
    parser.add_argument("--input", help="本地爬行动物全身高清图像或视频文件路径（建议侧面+俯视双角度，分辨率≥1080p）")
    parser.add_argument("--url", help="网络爬行动物全身高清图像/视频URL")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：爬宠蜕皮场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（饲养者/养殖场授权）")
    parser.add_argument("--list", action='store_true', help="显示爬宠蜕皮进度历史记录清单")
    ...
    try:
        ...
        print("🔍 正在分析爬宠蜕皮进度：体表颜色变化/眼部蓝眼状态/蜕皮阶段判定/卡皮风险评估，请稍候...")
