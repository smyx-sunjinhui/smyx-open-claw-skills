...
def main():
    parser = argparse.ArgumentParser(description="孕妇久站/过度劳累识别工具")
    parser.add_argument("--input", help="本地孕妇活动区域固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络孕妇活动区域固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：孕期健康管理场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示孕妇久站/过度劳累识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析孕妇姿态（连续站立时长 + 弯腰频次）并评估劳累风险，请稍候...")
