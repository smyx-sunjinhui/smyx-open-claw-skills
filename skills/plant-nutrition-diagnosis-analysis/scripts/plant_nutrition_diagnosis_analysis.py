...
def diagnose_nutrition(input_path=None, url=None, api_url=None, api_key=None, output_level=None):
...
def main():
    parser = argparse.ArgumentParser(description="植物营养诊断分析工具")
    parser.add_argument("--input", help="本地图片文件路径")
    parser.add_argument("--url", help="网络图片URL地址")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示植物营养诊断分析列表清单")
    ...
    try:
        ...
        print("🔍 正在诊断植物营养状况，请稍候...")
        output_content = diagnose_nutrition(
