...
def main():
    parser = argparse.ArgumentParser(description="中草药有效成分积累趋势评估工具")
    parser.add_argument("--input", help="本地药用植物叶片图像/视频文件路径")
    parser.add_argument("--url", help="网络药用植物叶片图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：中药材场景默认 other")
