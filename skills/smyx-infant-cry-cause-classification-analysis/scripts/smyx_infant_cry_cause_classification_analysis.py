...
def main():
    parser = argparse.ArgumentParser(description="婴幼儿哭声原因分类工具")
    parser.add_argument("--input", help="本地婴儿哭声音频/音视频文件路径")
    parser.add_argument("--url", help="网络婴儿哭声音频/音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：婴儿哭声分析场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示婴儿哭声原因分类分析列表清单")
