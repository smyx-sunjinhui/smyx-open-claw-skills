...
def main():
    parser = argparse.ArgumentParser(description="龟类张嘴呼吸（肺炎征兆）识别工具")
    parser.add_argument("--input", help="本地龟缸固定摄像头视频文件路径（建议正对头颈部高清拍摄，覆盖口鼻区域）")
    parser.add_argument("--url", help="网络龟缸固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：龟类肺炎征兆场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/养殖池/宠物医院授权）")
    parser.add_argument("--list", action='store_true', help="显示龟类肺炎风险预警历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测龟类非进食状态下口部开合频率与口鼻区域黏液/分泌物，评估肺炎风险征兆，请稍候...")
