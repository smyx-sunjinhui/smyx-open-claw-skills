...
def main():
    parser = argparse.ArgumentParser(description="儿童分离焦虑识别（上学前哭闹）工具")
    parser.add_argument("--input", help="本地家庭/幼儿园门口固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络家庭/幼儿园门口固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童情绪行为分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示儿童分离焦虑识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在识别儿童哭闹表情/肢体抓拽/抗拒行为并综合评估分离焦虑等级，请稍候...")
