...
def main():
    parser = argparse.ArgumentParser(description="成人心率变异性（HRV）趋势监测（面部）工具")
    parser.add_argument("--input", help="本地面部视频文件路径（建议 30-60 秒静坐正面视频）")
    parser.add_argument("--url", help="网络面部视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：成人面部 HRV 场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示成人 HRV 趋势分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析面部 rPPG 信号并计算 HRV 指标，请稍候...")
