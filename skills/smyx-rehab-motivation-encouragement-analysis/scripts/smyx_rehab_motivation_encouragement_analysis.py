...
def main():
    parser = argparse.ArgumentParser(description="康复患者沮丧/放弃倾向激励工具")
    parser.add_argument("--input", help="本地康复中心/家庭康复区固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络康复中心/家庭康复区固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：康复激励场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（患者本人或家属/康复师授权）")
    parser.add_argument("--list", action='store_true', help="显示康复患者沮丧/放弃倾向激励历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测康复患者沮丧/放弃倾向信号（叹气/中断训练/低头不语/动作迟缓/长期无进展）并触发激励反馈，请稍候...")
        ...
        print(f"❌ 康复患者沮丧/放弃倾向激励分析失败: {str(e)}")
        exit(1)
