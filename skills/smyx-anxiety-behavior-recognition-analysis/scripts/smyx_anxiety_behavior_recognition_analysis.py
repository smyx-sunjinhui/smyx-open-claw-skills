...
def main():
    parser = argparse.ArgumentParser(description="焦虑症相关行为（搓手、咬指甲、来回踱步）识别工具")
    parser.add_argument("--input", help="本地家庭/办公室固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络家庭/办公室固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：心理健康行为分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示焦虑症相关行为识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在识别手部搓揉/指甲啃咬/来回踱步等焦虑相关行为并计算焦虑行为指数，请稍候...")
