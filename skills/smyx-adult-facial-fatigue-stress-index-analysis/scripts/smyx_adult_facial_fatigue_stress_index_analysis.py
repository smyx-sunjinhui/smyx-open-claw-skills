...
def main():
    parser = argparse.ArgumentParser(description="成人面部疲劳/压力指数分析工具")
    parser.add_argument("--input", help="本地成人正面面部图像/短视频文件路径")
    parser.add_argument("--url", help="网络成人正面面部图像/短视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：成人个人健康状态场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示成人面部疲劳/压力指数分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析成人面部疲劳/压力指数（眼袋/黑眼圈/嘴角/皱眉纹），请稍候...")
