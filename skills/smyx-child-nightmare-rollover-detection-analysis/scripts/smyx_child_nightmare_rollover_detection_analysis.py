...
def main():
    parser = argparse.ArgumentParser(description="儿童睡眠中频繁翻身/噩梦识别工具")
    parser.add_argument("--input", help="本地儿童夜间睡眠音视频文件路径")
    parser.add_argument("--url", help="网络儿童夜间睡眠音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：儿童睡眠健康场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示儿童夜间翻身/噩梦识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析儿童夜间翻身/哭声/梦话并评估睡眠质量，请稍候...")
