...
def main():
    parser = argparse.ArgumentParser(description="鱼类聚集/离群行为识别工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头连续视频文件路径（用于鱼群质心+离群距离跟踪）")
    parser.add_argument("--url", help="网络鱼缸固定摄像头连续视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：鱼类聚集/离群行为识别场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养鱼用户、养殖场、场馆或检疫缸管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示鱼类聚集/离群行为识别历史记录清单")
    ...
    try:
        ...
        print("🔍 正在跟踪鱼群质心并计算每条鱼离群距离（体长倍数），判定持续离群行为，请稍候...")
