...
def main():
    parser = argparse.ArgumentParser(description="上班族下班疲劳关怀（回家时刻）工具")
    parser.add_argument("--input", help="本地客厅/单身公寓/家庭起居室固定摄像头回家后30分钟视频文件路径")
    parser.add_argument("--url", help="网络客厅/单身公寓/家庭起居室固定摄像头回家后30分钟视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：职场健康关怀场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号（用户本人授权）")
    parser.add_argument("--list", action='store_true', help="显示上班族下班疲劳关怀（回家时刻）历史关怀记录清单")
    ...
    try:
        ...
        print("🔍 正在检测瘫坐/斜躺/眼袋下垂嘴角/频繁眨眼/叹气并计算疲劳指数，请稍候...")
        ...
        print(f"❌ 上班族下班疲劳关怀（回家时刻）失败: {str(e)}")
        exit(1)
