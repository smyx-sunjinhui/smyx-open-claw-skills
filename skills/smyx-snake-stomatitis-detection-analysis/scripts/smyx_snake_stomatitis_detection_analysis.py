...
def main():
    parser = argparse.ArgumentParser(description="蛇类口腔腐肉识别（口炎）工具")
    parser.add_argument("--input",
                        help="本地蛇张口瞬间口腔内部高清图像或视频文件路径（建议正对口腔，光照充足，分辨率≥1080p）")
    parser.add_argument("--url", help="网络蛇张口瞬间口腔内部高清图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：蛇类口炎场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/爬宠医院/繁殖场授权）")
    parser.add_argument("--list", action='store_true', help="显示蛇类口炎风险评估历史记录清单")
    ...
    try:
        ...
        print(
            "🔍 正在分析蛇张口瞬间口腔内部高清图像，检测口腔黏膜颜色、脓点、溃疡、腐肉等病变特征，评估口炎风险等级，请稍候...")
