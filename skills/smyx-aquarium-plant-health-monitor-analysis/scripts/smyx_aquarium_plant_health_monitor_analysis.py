...
def main():
    parser = argparse.ArgumentParser(description="水族箱水草健康监测工具")
    parser.add_argument("--input", help="本地水族箱水草图像或视频文件路径")
    parser.add_argument("--url", help="网络水族箱水草图像或视频URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="对象类型，水族场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示水族箱水草健康监测历史报告清单")
