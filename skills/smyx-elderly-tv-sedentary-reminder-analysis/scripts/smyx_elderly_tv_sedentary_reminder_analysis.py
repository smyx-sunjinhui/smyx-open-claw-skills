...
def main():
    parser = argparse.ArgumentParser(description="老年人电视观看时长与久坐关联工具")
    parser.add_argument("--input", help="本地客厅沙发+电视区域视频文件路径")
    parser.add_argument("--url", help="网络客厅沙发+电视区域视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人健康生活方式场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人电视观看与久坐分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析老人沙发坐姿+面部朝向电视的连续观看时长（久坐提醒），请稍候...")
        ...
        print(f"❌ 老年人电视观看时长与久坐关联分析失败: {str(e)}")
        exit(1)
