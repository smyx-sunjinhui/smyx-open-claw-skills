...
def analyze_video(input_path=None, url=None, duration_min=None, api_url=None, api_key=None, output_level=None):
    """调用API进行宠物行为识别"""
    ...
    if duration_min and duration_min > 0:
        ConstantEnum.DEFAULT_DURATION_MIN = duration_min
...
def format_result(result, output_level="standard", duration_min=0):
    """格式化输出结果"""
    behavior_map = {
        "scratching": "抓挠",
        "chewing": "啃咬",
    ...
    if output_level == "json":
        ...
        if result is not None:
            ...
            result_json = json.dumps(result_json.get('behaviorDetectionResponse', {}), ensure_ascii=False, indent=2)
        else:
            return "⚠️ 暂无分析结果"
        return f"""
📊 宠物行为识别分析结构化结果
...
""", result_id
        ...
        abnormal_count = sum(v.get('count', 0) for k, v in behavior_stats.items() if
                             k in ['scratching', 'chewing', 'destroying', 'anxiety'])
        return f"""
📊 宠物行为识别报告
{'=' * 40}
统计时长: {duration_min if duration_min > 0 else '自动识别'} 分钟
异常行为次数: {abnormal_count}
主要异常行为: {', '.join([behavior_map.get(k, k) for k, v in behavior_stats.items() if v.get('count', 0) > 0])}
        """
        ...
        behaviors = "\n".join(
            [f"  {behavior_map.get(k, k)}: {v.get('count', 0)} 次，占比 {v.get('duration_percent', 0)}%" for k, v in
             behavior_stats.items()])
...
🔍 行为统计:
  总异常行为次数: {abnormal_count}

  各类行为统计:
{behaviors if behaviors else '  ✅ 未识别到明显行为'}

💡 建议提示:
{suggestions}
{'=' * 50}
> 注：本报告仅供宠物行为参考，行为矫正请咨询专业宠物训练师。
        """
    else:
        # 完整输出（JSON格式）
...
def main():
    parser = argparse.ArgumentParser(description="宠物行为识别工具")
    parser.add_argument("--input", help="本地视频文件路径")
    parser.add_argument("--url", help="网络视频的URL地址")
    parser.add_argument("--duration-min", type=float, default=ConstantEnum.DEFAULT__DURATION_MIN,
                        help="统计时长（分钟），默认自动识别")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示宠物行为识别列表清单")
