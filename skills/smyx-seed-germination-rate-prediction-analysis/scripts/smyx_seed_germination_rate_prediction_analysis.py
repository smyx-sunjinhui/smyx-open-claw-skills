...
def main():
    parser = argparse.ArgumentParser(description="种子发芽率早期预测工具")
    parser.add_argument("--input", help="本地育苗盘土壤表面图像或视频文件路径")
    parser.add_argument("--url", help="网络育苗盘土壤表面图像或视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：种子发芽场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示种子发芽率预测分析列表清单")
