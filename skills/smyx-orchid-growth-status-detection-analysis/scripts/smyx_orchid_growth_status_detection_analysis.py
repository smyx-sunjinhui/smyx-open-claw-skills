...
def main():
    parser = argparse.ArgumentParser(description="兰花新芽/花梗/根系状态识别工具")
    parser.add_argument("--input", help="本地兰花图像或视频文件路径")
    parser.add_argument("--url", help="网络兰花图像或视频URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="对象类型，植物场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示兰花生长状态识别历史报告清单")
