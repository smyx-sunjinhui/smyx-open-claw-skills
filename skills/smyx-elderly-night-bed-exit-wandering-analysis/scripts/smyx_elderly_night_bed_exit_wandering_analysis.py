...
def main():
    parser = argparse.ArgumentParser(description="老年人夜间离床时长与徘徊识别工具")
    parser.add_argument("--input", help="本地卧室/走廊夜间监控视频文件路径")
    parser.add_argument("--url", help="网络卧室/走廊夜间监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人夜间监护场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人夜间离床/徘徊分析列表清单")
