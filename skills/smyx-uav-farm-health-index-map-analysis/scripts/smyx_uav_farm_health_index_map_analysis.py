...
def main():
    parser = argparse.ArgumentParser(description="无人机农田健康指数图生成工具")
    parser.add_argument("--input", help="本地无人机航拍正射影像/拼接图/视频文件路径")
    parser.add_argument("--url", help="网络无人机航拍正射影像/拼接图/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：农田航拍场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示农田健康指数图历史分析列表清单")
