...
def main():
    parser = argparse.ArgumentParser(description="驾驶员眨眼频率与闭眼时长检测工具")
    ...
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：驾驶员疲劳监测场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示驾驶员疲劳监测分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析驾驶员眼部状态（眨眼频率/闭眼时长/微睡眠），请稍候...")
        ...
        print(f"❌ 驾驶员眨眼频率与闭眼时长检测失败: {str(e)}")
        exit(1)
