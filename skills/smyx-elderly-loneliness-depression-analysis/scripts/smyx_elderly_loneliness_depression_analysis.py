...
def main():
    parser = argparse.ArgumentParser(description="老年人孤独/抑郁倾向行为分析工具")
    parser.add_argument("--input", help="本地独居老人活动区域固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络独居老人活动区域固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人心理健康监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示老年人孤独/抑郁倾向行为分析列表清单")
    ...
    try:
        ...
        print("🔍 正在检测老年人独处期间的发呆/叹气/自言自语等消极行为并评估情绪风险等级，请稍候...")
        ...
        print(f"❌ 老年人孤独/抑郁倾向行为分析失败: {str(e)}")
        exit(1)
