...
def main():
    parser = argparse.ArgumentParser(description="水族箱内氨氮中毒视觉预兆（鱼浮头）工具")
    parser.add_argument("--input", help="本地鱼缸固定摄像头视频文件路径（建议覆盖水面带 + 包含侧面观察角）")
    ...
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：水族箱氨氮中毒视觉预兆场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（养鱼者/水族馆/养殖池管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示水族箱氨氮中毒视觉预兆历史记录清单")
    ...
    try:
        ...
        print("🔍 正在检测鱼嘴探出水面/口部快速开合/鳃盖加剧等鱼浮头行为，评估氨氮中毒或缺氧风险，请稍候...")
