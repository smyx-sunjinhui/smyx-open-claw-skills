...
def main():
    parser = argparse.ArgumentParser(description="蛙类皮肤湿润度评估工具")
    parser.add_argument("--input",
                        help="本地蛙类背部或侧身皮肤高清图像或视频文件路径（建议俯拍/侧拍，分辨率≥1080p，光照均匀避免眩光）")
    parser.add_argument("--url", help="网络蛙类背部或侧身皮肤高清图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：蛙类皮肤湿润度场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/两栖养殖场/宠物医院授权）")
    parser.add_argument("--list", action='store_true', help="显示蛙类皮肤脱水风险历史记录清单")
    ...
    try:
        ...
        print("🔍 正在分析蛙类背部/侧身皮肤光泽度、皱褶、白膜，对比物种健康基线，评估脱水风险，请稍候...")
