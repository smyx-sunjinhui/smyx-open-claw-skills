...
def main():
    parser = argparse.ArgumentParser(description="独居老人孤独情绪识别与温暖陪伴工具")
    parser.add_argument("--input", help="本地独居老人客厅/卧室/养老院单人房固定摄像头视频文件路径")
    parser.add_argument("--url", help="网络独居老人客厅/卧室/养老院单人房固定摄像头视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人心理健康关怀场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示独居老人孤独情绪识别与温暖陪伴历史报告列表清单")
    ...
    try:
        ...
        print("🔍 正在统计连续独处时长/静止发呆/叹气/自言自语并计算孤独指数，请稍候...")
        ...
        print(f"❌ 独居老人孤独情绪识别与温暖陪伴失败: {str(e)}")
        exit(1)
