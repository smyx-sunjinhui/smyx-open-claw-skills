...
def main():
    parser = argparse.ArgumentParser(description="守宫/蜥蜴尾巴断尾识别工具")
    parser.add_argument("--input", help="本地守宫/蜥蜴尾部高清图像或视频文件路径（建议侧位或俯拍清晰展示完整尾部）")
    parser.add_argument("--url", help="网络守宫/蜥蜴尾部图像/视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：爬宠断尾场景默认 other")
    parser.add_argument("--open-id", required=True,
                        help="当前用户的OpenID/UserId/用户名/手机号（饲养者/繁殖场管理员授权）")
    parser.add_argument("--list", action='store_true', help="显示守宫/蜥蜴断尾事件历史记录清单")
    ...
    try:
        ...
        print("🔍 正在测量尾部长度并与历史/体长基线对比，检测断尾事件、断端伤口与结痂情况，请稍候...")
