...
def main():
    parser = argparse.ArgumentParser(description="驾驶员面部潮红/出汗异常检测工具")
    ...
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：驾驶员健康监测场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示驾驶员面部潮红/出汗异常分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析驾驶员面部潮红指数与出汗反光面积，请稍候...")
        ...
        print(f"❌ 驾驶员面部潮红/出汗异常检测失败: {str(e)}")
        exit(1)
