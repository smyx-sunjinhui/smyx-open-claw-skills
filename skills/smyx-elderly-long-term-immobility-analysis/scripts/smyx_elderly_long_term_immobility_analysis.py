...
def main():
    parser = argparse.ArgumentParser(description="老年人长期静止（超12小时）监测工具")
    parser.add_argument("--input", help="本地居家多区域监控视频文件路径")
    parser.add_argument("--url", help="网络居家多区域监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：独居老人监护场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示老年人长期静止监测分析列表清单")
