...
def analyze_media(input_path=None, url=None, media_type=None, threshold=None, api_url=None, api_key=None,
                  output_level=None):
    """调用API进行人体情绪识别"""
...
def format_result(result, output_level="standard", media_type="video", threshold=0.7):
    """格式化输出结果"""
    emotion_map = {
        "happy": "快乐",
        "sad": "悲伤",
    ...
    if output_level == "json":
        ...
        if result is not None:
            ...
            result_json = json.dumps(result_json.get('emotionRecognitionResponse', {}), ensure_ascii=False, indent=2)
        else:
            return "⚠️ 暂无分析结果"
        return f"""
📊 人体情绪识别分析结构化结果
...
""", result_id
        ...
        dominant_emotion = recognition.get('dominant_emotion', '未知')
        dominant_emotion_cn = emotion_map.get(dominant_emotion, dominant_emotion)
        abnormal_emotions = recognition.get('abnormal_emotions', [])
        return f"""
📊 人体情绪识别报告
{'=' * 40}
主导情绪: {dominant_emotion_cn}
异常情绪数: {len(abnormal_emotions)}
情绪评分: {recognition.get('overall_score', '未知')}
        """
        ...
        emotions = "\n".join(
            [f"  {emotion_map.get(e.get('emotion'), e.get('emotion'))}: {e.get('probability'):.2f}" for e in
             recognition.get('emotions', [])])
        abnormal_emotions = "\n".join(
            [f"  ⚠️  {emotion_map.get(e, e)} (强度超过阈值)" for e in recognition.get('abnormal_emotions', [])])
        dominant_emotion = recognition.get('dominant_emotion', '未知')
        dominant_emotion_cn = emotion_map.get(dominant_emotion, dominant_emotion)
...
🔍 识别结果:
  主导情绪: {dominant_emotion_cn}
  整体情绪评分: {recognition.get('overall_score', '未知')}

  各情绪概率:
{emotions if emotions else '  未识别到有效情绪'}

  异常情绪标记:
{abnormal_emotions if abnormal_emotions else '  ✅ 未检测到异常情绪'}
{'=' * 50}
> 注：本报告仅供情绪状态参考，不能替代专业心理咨询和诊断，发现持续异常请及时寻求专业帮助。
        """
    else:
        # 完整输出（JSON格式）
...
def main():
    parser = argparse.ArgumentParser(description="人体视觉情绪识别工具")
    ...
    parser.add_argument("--threshold", type=float, default=ConstantEnum.DEFAULT__THRESHOLD,
                        help="异常情绪强度阈值，高于此分值标记为异常，默认 0.7")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示人体情绪识别列表清单")
