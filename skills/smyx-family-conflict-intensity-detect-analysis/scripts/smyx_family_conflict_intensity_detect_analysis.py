...
def main():
    parser = argparse.ArgumentParser(description="夫妻/家庭争吵强度识别工具")
    parser.add_argument("--input", help="本地客厅固定摄像头（含麦克风）音视频文件路径")
    parser.add_argument("--url", help="网络客厅固定摄像头（含麦克风）音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：家庭情绪与冲突分析场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示夫妻/家庭争吵强度识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析声音强度（分贝）与肢体动作激烈程度，综合评估家庭争吵冲突强度等级，请稍候...")
