...
def main():
    parser = argparse.ArgumentParser(description="老年人呼吸急促/困难识别工具")
    parser.add_argument("--input", help="本地老年人静息状态胸腹部监控视频文件路径")
    parser.add_argument("--url", help="网络老年人静息状态胸腹部监控视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：老年人呼吸健康监测场景默认 other")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示老年人呼吸急促识别分析列表清单")
    ...
    try:
        ...
        print("🔍 正在分析老年人静息呼吸频率并评估呼吸急促风险，请稍候...")
